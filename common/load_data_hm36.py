...
from common.reorder_joints import Reorder_Joints

# data_3d_h36m.npz, data_2d_h36m_gt.npz, data_2d_h36m_cpn_ft_h36m_dbb.npz  do not do any downsample action
...
class Fusion(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True):
        ...
        if self.train:
            ...
            self.cameras_train, self.poses_train, self.poses_train_2d = self.fetch(dataset, self.train_list, subset=self.subset)
            self.poses_train, self.poses_train_2d = Reorder_Joints('train', 'h36m', self.poses_train, self.poses_train_2d) # Reorder Joints
            new_kps_left = [8, 9, 10, 11, 12, 13]
            new_kps_right = [14, 15, 16, 5, 6, 7]
            new_joints_left = [8, 9, 10, 11, 12, 13]
            new_joints_right = [14, 15, 16, 5, 6, 7]
            self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_train, self.poses_train,
                                              ...
                                              kps_left=new_kps_left, kps_right=new_kps_right,
                                              joints_left=new_joints_left,
                                              joints_right=new_joints_right, out_all=opt.out_all)
            print('INFO: Training on {} frames'.format(self.generator.num_frames()))
        else:
            self.keypoints = self.prepare_data(dataset, self.test_list)
            self.cameras_test, self.poses_test, self.poses_test_2d = self.fetch(dataset, self.test_list, subset=self.subset)
            self.poses_test, self.poses_test_2d = Reorder_Joints('test', 'h36m', self.poses_test, self.poses_test_2d) # Reorder Joints
            new_kps_left = [8, 9, 10, 11, 12, 13]
            new_kps_right = [14, 15, 16, 5, 6, 7]
            new_joints_left = [8, 9, 10, 11, 12, 13]
            new_joints_right = [14, 15, 16, 5, 6, 7]
            self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_test, self.poses_test,
                                              self.poses_test_2d,
                                              pad=self.pad, augment=False, kps_left=new_kps_left,
                                              kps_right=new_kps_right, joints_left=new_joints_left,
        ...
        for subject in folder_list:
            ...
            for action in dataset[subject].keys():
                ...
                for cam_idx in range(len(keypoints[subject][action])):
                    ...
                    # print(subject, action, keypoints[subject][action][cam_idx].shape[0])
    ...
    def get_generator_random_state(self): # 我加的
        random_state = self.generator.random_state()
        return random_state
