...
class Fusion(data.Dataset):
    def __init__(self, opt, root_path, train=True):
        ...
        self.pad = opt.pad
        if self.train:
            self.poses_train, self.poses_train_2d = self.prepare_data(opt.root_path, train=True, stride=opt.downsample)
            # print('self.poses_train : ', self.poses_train[('S1', 'Seq1', '0')].shape) 
            self.generator = ChunkedGenerator(batch_size = opt.batch_size // opt.stride,
                                              cameras = None,
                                              poses_3d = self.poses_train,
                                              poses_2d = self.poses_train_2d,
                                              valid_frame = None,
                                              chunk_length = self.stride,
                                              pad = self.pad,
                                              shuffle = True,
            ...
            self.poses_test, self.poses_test_2d, self.valid_frame = self.prepare_data(opt.root_path, train=False, stride=opt.downsample)
            print('self.valid_frame : ', self.valid_frame['TS1'])
            self.generator = ChunkedGenerator(batch_size = opt.batch_size // opt.stride,
                                              cameras = None,
    ...
    def prepare_data(self, path, train=True, stride=5):
        out_poses_3d = {}
        out_poses_2d = {}
        valid_frame = {}
        ...
        if train == True:
            ...
            # print('out_poses_3d : ', out_poses_3d[('S1', 'Seq1', '0')].shape) 
            for key in out_poses_2d.keys():
                out_poses_2d[key] = out_poses_2d[key][::stride]
                out_poses_3d[key] = out_poses_3d[key][::stride]
            # print('out_poses_3d : ', out_poses_3d[('S1', 'Seq1', '0')].shape) 
    ...
    def __getitem__(self, index):
        ...
        cam, gt_3D, input_2D, seq, subject, cam_ind = self.generator.get_batch(seq_name, start_3d, end_3d, flip,
                                                                               reverse)
