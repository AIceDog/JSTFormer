...
class CHI_Block(nn.Module):
    ...
    def forward(self, x_1, x_2, x_3):
        ...
        x = torch.cat([x_1, x_2, x_3], dim=2) # input shape: x_1, x_2, x_3 : [B, F, 256], output shape : x : [B, F, 256 * 3]
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.linear_1 = nn.Linear(5 * args_channel, embed_dim, bias=qkv_bias)
        self.linear_2 = nn.Linear(6 * args_channel, embed_dim, bias=qkv_bias)
        self.linear_3 = nn.Linear(6 * args_channel, embed_dim, bias=qkv_bias)
        
        self.pos_embed_1 = nn.Parameter(torch.zeros(1, frame_length, embed_dim))
        self.pos_embed_2 = nn.Parameter(torch.zeros(1, frame_length, embed_dim))
        self.pos_embed_3 = nn.Parameter(torch.zeros(1, frame_length, embed_dim))

        self.pos_drop_1 = nn.Dropout(p=drop_rate)
        self.pos_drop_2 = nn.Dropout(p=drop_rate)
        self.pos_drop_3 = nn.Dropout(p=drop_rate)
        
        self.linear_out_1 = nn.Linear(embed_dim, 5 * args_channel, bias=qkv_bias)
        self.linear_out_2 = nn.Linear(embed_dim, 6 * args_channel, bias=qkv_bias)
        self.linear_out_3 = nn.Linear(embed_dim, 6 * args_channel, bias=qkv_bias)
        ...
        self.norm = norm_layer(args_channel * 17)
    ...
    def forward(self, x_1, x_2, x_3):
        ...
        x_1 += self.pos_embed_1 # torso
        x_2 += self.pos_embed_2 # lArm, rLeg
        x_3 += self.pos_embed_3 # rArm, lLeg

        x_1 = self.pos_drop_1(x_1)
        x_2 = self.pos_drop_2(x_2)
        x_3 = self.pos_drop_3(x_3)
        
        x_1, x_2, x_3 = self._CHI_Block(x_1, x_2, x_3) # input shape: x_1, x_2, x_3 : [B, F, 256]
        
        x_1 = self.linear_out_1(x_1)
        x_2 = self.linear_out_2(x_2)
        x_3 = self.linear_out_3(x_3)
