...
def compute_bone_length_loss(predicted, target):
    assert predicted.shape == target.shape
    B, F, J, C = predicted.shape
    joint_index1 = [0, 1, 2, 3, 0, 8,  9,  0, 14, 15,  2, 11, 12, 2, 5, 6] 
    joint_index2 = [1, 2, 3, 4, 8, 9, 10, 14, 15, 16, 11, 12, 13, 5, 6, 7]
    
    boneLen_loss = 0
    
    for i in range(len(joint_index1)):
        boneLen_loss += torch.abs(torch.sum(torch.pow(predicted[: , :, joint_index1[i], :] - predicted[: , :, joint_index2[i], :], 2)) - torch.sum(torch.pow(target[: , :, joint_index1[i], :] - target[: , :, joint_index2[i], :], 2)))

    boneLen_loss = boneLen_loss / (B * F)
...
def save_model(previous_name, save_dir, epoch, data_threshold, model, lr, train_generator_random_state, optimizer): # lr, train_generator, optimizer 是我加的
    ...
    torch.save({
    'epoch': epoch, # 我加的 this epoch, not next epoch
    'lr': lr, # 我加的 lr for this epoch
    'random_state': train_generator_random_state, # 我加的
    'optimizer': optimizer.state_dict(), # 我加的 optimizer for this epoch
    'model': model.state_dict(),
    'previous_best_threshold' : data_threshold, # 我加的
    'previous_name' : previous_name, # 我加的
    }, '%s/model_%d_%d.pth' % (save_dir, epoch, data_threshold * 100))
    
    return previous_name
    

def save_model_epoch(save_dir, epoch, model, lr, train_generator_random_state, optimizer, previous_best_threshold, previous_name): # lr, train_generator, optimizer 是我加的
    torch.save({
        'epoch': epoch, # 我加的 this epoch, not this epoch
        'lr': lr, # 我加的 lr for this epoch
        'random_state': train_generator_random_state, # 我加的
        'optimizer': optimizer.state_dict(), # 我加的 optimizer for this epoch
        'model': model.state_dict(),
        'previous_best_threshold' : previous_best_threshold, # 我加的
        'previous_name' : previous_name, # 我加的
    }, '%s/epoch_%d.pth' % (save_dir, epoch))


def save_model_3dhp(previous_name, save_dir,epoch, data_threshold, model, model_name):
    # if os.path.exists(previous_name):
    #     os.remove(previous_name)
